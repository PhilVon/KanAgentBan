...
search "q"   → ranked one-line hits across tasks/docs/comments/ideas (snippets)
brainstorm show B-n → clustered, score-ranked ideas (sheds lowest clusters)
```
...
- Versioned: `--format-version` (current `11`); changes bump the version so a
  ...
  **v10** added the `search` tier (budgeted one-line ranked hits); **v11** added
  the brainstorm tier (clustered score-ranked `brainstorm show`, session list,
  `context` open-session anchor) and idea search hits.
